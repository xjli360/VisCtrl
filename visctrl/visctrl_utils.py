...
def regiter_attention_editor_diffusers(model, editor: AttentionBase):
    ...
    def ca_forward(self, place_in_unet):
        def forward(x, encoder_hidden_states=None, attention_mask=None, context=None, mask=None):
            ...
            # x = [x_ref, x_tar, x_ref, x_tar]   context = [uc, uc, ref, tar]

            h = self.heads  # x = [4,4096,32]
            # q = [q_tar_ref, q_tar_cur, q_tar_ref, q_tar_cur] -> [q_ref, q_tar, q_ref, q_tar]
            q = self.to_q(x)

            is_cross = context is not None
            context_i = context if is_cross else x
            # context = context if is_cross else torch.cat([x[:1]]*8)

            # k = [k_uncond, k_uncond, k_src, k_tar]
            # q,k,v [4,4096,320]
            k = self.to_k(context_i)
            v = self.to_v(context_i)
            # q,k,v [8,4096,320] -> [64,4096,40]
            q, k, v = map(lambda t: rearrange(t, 'b n (h d) -> (b h) n d', h=h), (q, k, v))


            sim = torch.einsum('b i d, b j d -> b i j', q, k) * self.scale  # [64,4096,4096]
            ...
            attn = sim.softmax(dim=-1)  # [64,4096,4096]

            # context_o = torch.cat([context[-3:-2]]*8) if is_cross else x
            # q_o = self.to_q(x)
            # k_o = self.to_k(context_o)
            # v_o = self.to_v(context_o)
            # q_o, k_o, v_o = map(lambda t: rearrange(t, 'b n (h d) -> (b h) n d', h=h), (q_o, k_o, v_o))
            # sim_o = torch.einsum('b i d, b j d -> b i j', q_o, k_o) * self.scale
            # attn_o = sim_o.softmax(dim=-1)
